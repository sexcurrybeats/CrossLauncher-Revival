...
class XMBWaveSettingSubDialog(v : XmbView) : XmbDialogSubview(v) {
    ...
    * */
    ...
    private val monthlyColorsTop = intArrayOf(
        0xB0B0B0, 0xCA9E0A, 0x7DA91E, 0xE84A7B,
        0x137015, 0x6855A4, 0x137015, 0x00074F,
        0xAB42C3, 0xE49807, 0x774917, 0xD33C29
    )
    private val monthlyColorsBottom = intArrayOf(
        0xCCCCCC, 0xD5DD32, 0xA8B821, 0xDC9CAC,
        0x179A18, 0xC282D9, 0x0DE8E4, 0x0AB0E0,
        0xCA5DDC, 0xD7C501, 0x98732D, 0xE5432C
    )

    private fun getMonthlyColor(isTop: Boolean, month: Int): Int {
        val m = if (month == 0) {
            Calendar.getInstance().get(Calendar.MONTH)
        } else {
            (month - 1) % 12
        }.coerceIn(0, 11)
        val color = if (isTop) monthlyColorsTop[m] else monthlyColorsBottom[m]
    ...
    private fun readWallpaperPreferences() {
        ...
        state.month = pref.getInt(XMBWaveSurfaceView.KEY_MONTH, id.psw.vshlauncher.submodules.settings.WaveSettings.DEFAULT_WAVE_MONTH)
        state.speed = pref.getFloat(XMBWaveSurfaceView.KEY_SPEED, id.psw.vshlauncher.submodules.settings.WaveSettings.DEFAULT_WAVE_SPEED)
        state.style = pref.getInt(XMBWaveSurfaceView.KEY_STYLE, id.psw.vshlauncher.submodules.settings.WaveSettings.DEFAULT_WAVE_STYLE.toInt())
    ...
    private fun updateColorPaintGradient(isFg: Boolean){
        val colors = if(isFg){
            ...
            if(state.month == -1) {
                ...
                intArrayOf(
                    getMonthlyColor(true, state.month),
                    getMonthlyColor(false, state.month)
                )
            }
        }
