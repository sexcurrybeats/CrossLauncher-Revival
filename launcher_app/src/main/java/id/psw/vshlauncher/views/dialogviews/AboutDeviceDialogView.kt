...
class AboutDeviceDialogView(v: XmbView) : XmbDialogSubview(v) {
    companion object {
        private const val ALLEY_HOOPS_URL = "https://spliffcurrybeats.itch.io/alley-hoops-confidential-test"
    }
    ...
    private var trianglePressCount = 0
    ...
    override fun onGamepad(key: PadKey, isPress: Boolean): Boolean {
        ...
        if (key != PadKey.Triangle) {
            trianglePressCount = 0
            return false
        }

        trianglePressCount++
        if (trianglePressCount >= 5) {
            trianglePressCount = 0
            vsh.postNotification(R.drawable.icon_info, "Unlocked", "Opening Alley Hoops...")
            vsh.openUrlOrNotify(ALLEY_HOOPS_URL, R.string.error_no_browser_description)
        }
        return true
    }
