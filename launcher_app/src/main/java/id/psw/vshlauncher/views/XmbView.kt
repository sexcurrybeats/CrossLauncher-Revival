...
class XmbView @JvmOverloads constructor(
    ...
    companion object{
        ...
        private const val DEFAULT_RENDER_FPS = 60L
        private const val IDLE_RENDER_FPS = 30L
        private const val MIN_RENDER_FPS = 15L
        private const val MAX_RENDER_FPS = 120L
        private const val RECENT_INTERACTION_FULL_FPS_MS = 500L
    }
    ...
    var fpsLimit = DEFAULT_RENDER_FPS
    ...
    private var lastAdaptedWidth = -1
    private var lastAdaptedHeight = -1
    private var lastKeygenDateCheckAt = 0L
    private var shouldDrawKeygenText = false
    private var lastInteractionAt = 0L

    private fun doNothing(){}

    private fun shouldUseFullFrameRate(): Boolean {
        if (SystemClock.uptimeMillis() - lastInteractionAt < RECENT_INTERACTION_FULL_FPS_MS) return true
        if (!::activeScreen.isInitialized) return true
        return activeScreen.prefersFullFrameRate()
    }

    private fun targetFrameDurationMs(): Long {
        val configuredFps = fpsLimit.takeIf { it > 0L } ?: DEFAULT_RENDER_FPS
        val targetFps = if (shouldUseFullFrameRate()) configuredFps else configuredFps.coerceAtMost(IDLE_RENDER_FPS)
        return (1000L / targetFps.coerceIn(MIN_RENDER_FPS, MAX_RENDER_FPS)).coerceAtLeast(1L)
    }

    private fun sleepToFrameLimit(frameStartedAt: Long) {
        val remaining = targetFrameDurationMs() - (SystemClock.uptimeMillis() - frameStartedAt)
    ...
    private fun drawThreadFunc(){
        if(!isRenderThreadRunning){
            ...
            while(!holder.surface.isValid && shouldKeepRenderThreadRunning){
                ...
                doNothing()
                sleepToFrameLimit(frameStartedAt)
            }
    ...
    private fun loadPreferences(){
        ...
        screens.mainMenu.layoutMode = when(pref.get(PrefEntry.MENU_LAYOUT, 1)){
            0 -> XmbLayoutType.PS3
            1 -> XmbLayoutType.PSP
            2 -> XmbLayoutType.Bravia
            3 -> XmbLayoutType.PSX
            else -> XmbLayoutType.PSP
        }
    ...
    private fun start(){
        ...
        // Offload initial scanning to after first frame setup
        context.vsh.M.apps.reloadAppList()
        context.vsh.M.media.mediaListingStart()
    }

    private fun adaptScreenSize(){
        if (lastAdaptedWidth == width && lastAdaptedHeight == height) return
        lastAdaptedWidth = width
        lastAdaptedHeight = height
    ...
    fun showSideMenu(open: Boolean = true) {
        if (open) widgets.sideMenu.show() else widgets.sideMenu.hide()
        widgets.sideMenu.selectedIndex = context.vsh.hoveredItem?.setMenuOpened(open) ?: 0
    }

    fun showSideMenu(items: ArrayList<XmbMenuItem>) {
        widgets.sideMenu.show(items)
    }
    ...
    fun onGamepadInput(key: PadKey, isDown:Boolean) : Boolean{
        if (isDown) lastInteractionAt = SystemClock.uptimeMillis()
        if(widgets.sideMenu.isDisplayed){
    ...
    fun onUpdate(){
        context.vsh.itemOffsetX = (time.deltaTime * 18.0f).coerceIn(0.0f, 1.0f).toLerp(context.vsh.itemOffsetX, 0.0f)
        context.vsh.itemOffsetY = (time.deltaTime * 18.0f).coerceIn(0.0f, 1.0f).toLerp(context.vsh.itemOffsetY, 0.0f)
    ...
    private fun drawKeygen(ctx: Canvas) {
        ...
        if (now - lastKeygenDateCheckAt >= 60_000L || lastKeygenDateCheckAt == 0L) {
            val cal = Calendar.getInstance()
            val mon = cal.get(Calendar.MONTH)
            val day = cal.get(Calendar.DAY_OF_MONTH)
            shouldDrawKeygenText = day == 1 && mon == 3
            lastKeygenDateCheckAt = now
        }

        // April Mob wkwkwkwkwkwk
        if(shouldDrawKeygenText){
