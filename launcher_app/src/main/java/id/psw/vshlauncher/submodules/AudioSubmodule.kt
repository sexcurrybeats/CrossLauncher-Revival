...
class AudioSubmodule(private val ctx : Vsh) : IVshSubmodule {
    companion object {
        ...
        private val SUPPORTED_MENU_SFX_EXTENSIONS = arrayOf("ogg", "wav", "mp3")
        private val SFX_RESOURCE_NAMES = linkedMapOf(
    ...
    private var pendingSfxPickType: SfxType? = null
    ...
    fun loadSfxData(attach : Boolean = true){
        ...
        for(it in SFX_RESOURCE_NAMES)
        {
            ...
            for(file in files){
                ...
                Logger.d(TAG, " ${it.key}::\"${file.absolutePath}\" found ? $found")
                if(found){
    ...
    fun openSfxPicker(type: SfxType) {
        pendingSfxPickType = type
        ctx.xmb.menuSfxPicker.launch("audio/*")
    }

    fun onSfxPicked(uri: Uri) {
        val type = pendingSfxPickType ?: return
        pendingSfxPickType = null
        importSfxOverride(type, uri)
            .onSuccess { file ->
                ctx.postNotification(
                    R.drawable.category_setting,
                    ctx.getString(R.string.common_success),
                    ctx.getString(R.string.settings_audio_sfx_pick_success, sfxLabel(type), file.name),
                    5.0f
                )
            }
            .onFailure { error ->
    ...
    fun resetSfxOverride(type: SfxType): Int {
        val deleted = sfxFiles(type).count { file ->
        ...
        ctx.postNotification(
            R.drawable.category_setting,
            ctx.getString(R.string.common_success),
            ctx.getString(R.string.settings_audio_sfx_reset_success, sfxLabel(type)),
            5.0f
        )
        return deleted
    }

    private fun importSfxOverride(type: SfxType, uri: Uri): Result<File> {
        return runCatching {
            val extension = resolveSfxImportExtension(uri)
            val tempFile = File.createTempFile("sfx_${sfxResourceName(type)}_", ".$extension", ctx.cacheDir)
            ...
            try {
                ...
                } ?: throw IllegalStateException(ctx.getString(R.string.error_audio_sfx_source_missing))

                sfxFiles(type).forEach { file ->
                ...
                val target = FileQuery(VshBaseDirs.VSH_RESOURCES_DIR)
                    ...
                    .withNames(sfxResourceName(type))
                    .withExtensions(extension)
                    .execute(ctx)
                    .firstOrNull()
                    ?: throw IllegalStateException(ctx.getString(R.string.error_audio_sfx_destination_missing))
    ...
    private fun sfxFiles(type: SfxType): List<File> {
    ...
    private fun resolveSfxImportExtension(uri: Uri): String {
        ...
        return candidates
            .map { normalizeSfxExtension(it) }
            .firstOrNull { candidate -> SUPPORTED_MENU_SFX_EXTENSIONS.any { it.equals(candidate, ignoreCase = true) } }
            ?: throw IllegalArgumentException(ctx.getString(R.string.error_audio_sfx_unsupported_type))
    }

    private fun normalizeSfxExtension(extension: String): String {
    ...
    private fun sfxResourceName(type: SfxType): String {
        return SFX_RESOURCE_NAMES[type] ?: throw IllegalArgumentException("Unsupported SFX type: $type")
    }

    fun sfxLabel(type: SfxType): String {
        return ctx.getString(
            when(type) {
                SfxType.Selection -> R.string.settings_audio_sfx_selection_label
                SfxType.Confirm -> R.string.settings_audio_sfx_confirm_label
                SfxType.Cancel -> R.string.settings_audio_sfx_cancel_label
            }
        )
    }
