...
class AudioSettings(private val vsh: Vsh) : ISettingsCategories(vsh) {
    ...
    private fun mkItemMenuSfxSubMenu(): XmbSettingsCategory {
        return XmbSettingsCategory(vsh, "settings_audio_menu_sfx",
            R.drawable.ic_component_audio,
            R.string.settings_audio_menu_sfx_name,
            R.string.settings_audio_menu_sfx_desc
        ).apply {
            content.addAllV(
                mkItemPickSfx(SfxType.Selection),
                mkItemPickSfx(SfxType.Confirm),
                mkItemPickSfx(SfxType.Cancel),
                mkItemResetSfx(SfxType.Selection),
                mkItemResetSfx(SfxType.Confirm),
                mkItemResetSfx(SfxType.Cancel)
            )
        }
    }

    private fun mkItemPickSfx(type: SfxType): XmbSettingsItem {
        val name = when(type) {
        ...
        return XmbSettingsItem(vsh, "audio_pick_sfx_${type.name.lowercase()}",
            name,
    ...
    private fun mkItemResetSfx(type: SfxType): XmbSettingsItem {
        val name = when(type) {
        ...
        return XmbSettingsItem(vsh, "audio_reset_sfx_${type.name.lowercase()}",
            name,
    ...
    override fun createCategory() : XmbSettingsCategory {
        return XmbSettingsCategory(vsh, "settings_category_audio",
