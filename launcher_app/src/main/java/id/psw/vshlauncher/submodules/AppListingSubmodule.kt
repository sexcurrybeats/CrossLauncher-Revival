...
import id.psw.vshlauncher.refreshRecentGamesNode
...
class AppListingSubmodule(private val vsh : Vsh) : IVshSubmodule {
    ...
    private fun appCategorySetSorting(it:XmbItemCategory, newSort:Any){
        if(newSort is AppItemSorting){
            fun sortContent(content: ArrayList<XmbItem>) {
                content.sortBy { item ->
                ...
                content.filterIsInstance<XmbNodeItem>().forEach { node ->
                    sortContent(node.content)
                }
            }

            sortContent(it.content)
        }
    }
                ...
                withContext(Dispatchers.IO){
                    ...
                    withContext(Dispatchers.Main) {
                        for (cat in vsh.categories) {
                            fun recursiveRemove(content: ArrayList<XmbItem>) {
                                synchronized(content) {
                                    val toRemove = content.filterIsInstance<XmbAppItem>()
                                    content.removeAll(toRemove.toSet())
                                    for(item in content){
                                        if(item is XmbNodeItem) recursiveRemove(item.content)
                                    }
                                }
                            }
                            recursiveRemove(cat.content)
                        }
                        vsh.allAppEntries.clear()
                    ...
                    val items = mutableListOf<XmbAppItem>()
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        items.forEach { item ->
                            if (vsh.allAppEntries.none { it.id == item.id }) {
                                vsh.allAppEntries.add(item)
                                val cat = vsh.categories.find { cc -> cc.id == item.appCategory }
                                if (cat == null) {
                                    val isGame = isAGame(item.resInfo)
                                    vsh.addToCategory(
                                        isGame.select(Vsh.ITEM_CATEGORY_GAME, Vsh.ITEM_CATEGORY_APPS),
                                        item
                                    )
                                } else {
                        ...
                        gameCat?.setSort(AppItemSorting.Name)
                        appCat?.setSort(AppItemSorting.Name)
                        vsh.refreshRecentGamesNode()
                    }
